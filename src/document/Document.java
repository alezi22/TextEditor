...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		char[] wa = word.toLowerCase().toCharArray();
		int regulator = 0;
		String vowels = "aeiouy";		
		for(int i = 0; i<wa.length; i++) {
			if(vowels.indexOf(wa[i]) >= 0) {
				if(i == 0 || vowels.indexOf(wa[i-1]) == -1) {
					regulator++;
				}				
			}
		}
		if(word.toLowerCase().endsWith("e") && vowels.indexOf(wa[wa.length-2]) == -1 && regulator > 1) {
			regulator--;
		}
	    return regulator;
	}
	...
	public double getFleschScore()
	{
		...
		double w = getNumWords();
		double s = getNumSentences();
		double y = getNumSyllables();
		double x = 1.015*(w/s);
		double z = 84.6*(y/w);
	    return 206.835-x-z;
	}
