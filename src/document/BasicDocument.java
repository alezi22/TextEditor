...
public class BasicDocument extends Document 
{
	...
	public int getNumSyllables()
	{
		...
		List<String> lone = getTokens("[^aeiouyAEIOUY]+[eE]\\b");
		List<String> single = getTokens("\\b[^aeiouyAEIOUY]*[eE]\\b");*/
        //return tokens.size()-(lone.size() - single.size());
		String[] w = getText().split("[^a-zA-Z]+");
		int total = 0;
		for(String t : w) {
			int x = countSyllables(t);			
			total += x;
		}		
        return total;
	}
